...
The speed of the animation between collapsed and expanded can be is an option that can be adjusted.
So can whether the sidebar is on the left or on the right.
...
<div style="display: grid; grid-template-columns: repeat(1, 1fr); gap: 15px; width: 50%; max-width: 600px;">
    ...
    <!-- undecided if I want a continuous controller button here or just the right-click context menu
    <div>
        <button style="width:3.5em; height:3.5em">
...
2. The second button is the [tuning menu](tuning.md).

Either or none of these can be active. The menu is collapsed if and only if none.
You can toggle between them if expanded.

3. The third button is the volume button. When pressed a volume slider appears. In addition there is a meter for the volume. The slider and the meter are parallel and use a common scale. CC7 is hardcoded for volume.
4. All sound off (and implicitly all notes off) button. CC120 is hardcoded to this control.


Beyond the sidebar, there is also the [right-click context menu](right-click.md) for dealing with continuous controller settings.
